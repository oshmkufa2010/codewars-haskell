...
foldExpr faa (In e) = faa $ fmap (foldExpr faa) e
...
class Functor f => Pretty f where
  prettyAlgebra :: Pretty g => f (Expr g) -> String

pretty :: Pretty f => Expr f -> String
pretty (In e) = prettyAlgebra e

instance Pretty Lit where
  prettyAlgebra (Lit x) = show x

instance Pretty Add where
  prettyAlgebra (Add x y) = "(" ++ pretty x ++ "+" ++ pretty y ++ ")"

instance Pretty Mult where
  prettyAlgebra (Mult x y) = "(" ++ pretty x ++ "*" ++ pretty y ++ ")"

instance (Pretty f, Pretty g) => Pretty (f :+: g) where
  prettyAlgebra (Inl fe) = prettyAlgebra fe
  prettyAlgebra (Inr ge) = prettyAlgebra ge
